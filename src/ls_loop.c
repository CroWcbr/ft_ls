...
/*   Updated: 2023/06/28 23:38:07 by cdarrell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	dir_loop(char* dir_path, t_flags* flags)
{
	...
	while ((entry = readdir(dir)) != NULL)
	{
		add_char_to_list((char*)entry->d_name, &this_dir_list);
		printf("%s\n", entry->d_name);
	}
	...
	if (dir)
	{
		...
		while (tmp_dir)
		{
			if (file)
			{
				ft_putstr("\n");
				if (!ls->flags.f_big_r)
					ft_putstr_n(((t_elem *)tmp_dir->content)->name, ":\n", "\0");
			}
			dir_loop(((t_elem *)tmp_dir->content)->name, &ls->flags);
